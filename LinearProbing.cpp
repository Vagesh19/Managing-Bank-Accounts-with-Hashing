...
void LinearProbing::merge(std::vector<int>& arr, int l , int m, int r) {
    int len1 = m - l + 1;
    int len2 = r - m;
...
void LinearProbing::ms(std::vector<int>& arr, int l, int r) {
    if (l < r) {
        int m = (r +l) / 2;
        ms(arr, l, m);
        ms(arr, m + 1, r);
...
int LinearProbing::powermod(int a, int b,int c){
    if (a==0){
        return 0;
    }
    if (b==0){
